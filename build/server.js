...
io.on('connection', (socket) => {
    var _a;
    console.log(`Usuario conectado al chat: ${socket.id}, UserId: ${(_a = socket.user) === null || _a === void 0 ? void 0 : _a.userId}`);
    ...
    socket.on('typing_in_combat', (data) => {
        if (!socket.user || !data || !data.combatId)
            return;
        const combatRoom = `combat_${data.combatId}`;
    ...
    socket.on('disconnect', (reason) => {
        var _a;
        console.log(`Usuario desconectado del chat: ${socket.id}, UserId: ${(_a = socket.user) === null || _a === void 0 ? void 0 : _a.userId}. Razón: ${reason}`);
        if (socket.user) {
    ...
    socket.on('error', (err) => {
        var _a;
        console.error(`Error en socket ${socket.id} (Usuario ${(_a = socket.user) === null || _a === void 0 ? void 0 : _a.userId}): ${err.message}`);
